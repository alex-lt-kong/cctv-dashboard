...
void* thread_capture_live_image(void* payload) {
    ...
    if (result == false) {
        syslog(LOG_ERR, "thread%2d | cap.read() failed, will cap.release() and "
            "then re-try after sleep(%d)", pl->tid, sleep_sec);
        cap.release();
    ...
    if (img_size > SHM_SIZE - 4) {
        syslog(LOG_ERR, "thread%2d | encoded_img.size() == %lu > SHM_SIZE == "
...
int main() {
    ...
    is >> json_settings;
    size_t videos_devices_len = json_settings["app"]["video_devices"].size();
    ...
    vector<struct CamPayload> cpls(videos_devices_len);
    pthread_t tids[videos_devices_len];
    for (size_t i = 0; i < videos_devices_len; ++i) {
        cpls[i].device_uri = strdup(json_settings["app"]["video_devices"][i].get<string>().c_str());
